...
namespace boop {
  ...
  class Helo {
  public:
    explicit Helo(const BoopHeloParams &params)
      : params_(params) {
    }

    void Run(Ntk *pNtk) const {
      RunWithPartitioner(pNtk);
    }
  ...
  private:
    ...
    typename HeloScheduler<Ntk, Opt, Prt>::Parameter MakeSchedulerParams() const {
      ...
      par.fnObjective = [this](Ntk *pNtk) {
        return GetTwoInputSize(pNtk);
      };
    ...
    void LowerAnalyzerParams(Par &par) const {
      par.nVerbose = params_.nAnalyzerVerbose;
    }

    template <typename Par>
    void LowerSimulatorParams(Par &par) const {
    ...
    template <typename Par>
    void LowerSatParams(Par &par) const {
      par.nVerbose = params_.nSatVerbose;
      par.nConflictLimit = params_.nConflictLimit;
    }

    template <typename Opt, typename Prt>
    void RunScheduler(Ntk *pNtk, const typename HeloScheduler<Ntk, Opt, Prt>::Parameter &params) const {
      HeloScheduler<Ntk, Opt, Prt> scheduler(pNtk, params);
      scheduler.Run();
    }

    template <typename Ana, typename Prt>
    void RunWithOptimizer(Ntk *pNtk, const typename Ana::Parameter &parAna) const {
      switch(params_.nOptimizer) {
      case 0: {
        using Opt = rrr::Optimizer<Ntk, Ana>;
        typename HeloScheduler<Ntk, Opt, Prt>::Parameter par = MakeSchedulerParams<Opt, Prt>();
    ...
    void RunWithAnalyzer(Ntk *pNtk, const typename Sim::Parameter &parSim, const typename Sat::Parameter &parSat) const {
    ...
    void RunWithSimulator(Ntk *pNtk, const typename Sat::Parameter &parSat) const {
    ...
    void RunWithSat(Ntk *pNtk) const {
      switch(params_.nSat) {
      case 0: {
    ...
    template <typename Prt>
    void RunWithAnalyzerPre(Ntk *pNtk) const {
      switch(params_.nAnalyzer) {
      case 0:
        RunWithSat<Prt>(pNtk);
        break;
