...
template <typename Ntk> int ReadAigString(const std::string &str, Ntk *pNtk) {
...
template <typename Ntk> std::string CreateAig(const Ntk *pNtk, int nLatches) {
  ...
  pNtk->ForEachInt([&](int nId) {
    if (pNtk->GetNumFanins(nId) > 1) {
      ...
      int n0 = vValues[pNtk->GetFanin(nId, i)] ^
               static_cast<int>(pNtk->GetCompl(nId, i));
      i--;
      int n1 = vValues[pNtk->GetFanin(nId, i)] ^
               static_cast<int>(pNtk->GetCompl(nId, i));
      i--;
      if (n0 < n1) {
        std::swap(n0, n1);
  ...
  ss0 << "aig " << nNodes - 1 << " " << pNtk->GetNumPis() - nLatches << " "
...
template <typename Ntk> int ReadAig(const std::string &filename, Ntk *pNtk) {
  std::stringstream ss;
