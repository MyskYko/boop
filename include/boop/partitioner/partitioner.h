...
template <typename Ntk> class Partitioner {
public:
  struct Parameter {
    int nVerbose = 0;
...
private:
  using PartitionIo = std::tuple<std::set<int>, std::vector<int>,
  ...
  // partition
  void GetIo(const std::set<int> &sNodes, std::set<int> &sInputs,
             std::set<int> &sOutputs);
  std::set<int> GetFanouts(const std::set<int> &sNodes,
                           const std::set<int> &sOutputs);
...
Partitioner<Ntk>::Partitioner(const Parameter &par)
    : pNtk_(nullptr), par_(par) {}

template <typename Ntk> void Partitioner<Ntk>::AssignNetwork(Ntk *pNtk) {
  pNtk_ = pNtk;
...
template <typename Ntk>
void Partitioner<Ntk>::SetPrintLine(
...
template <typename Ntk> Ntk *Partitioner<Ntk>::Extract(int nSeed) {
  ...
  for (int i = 0; i < int_size(vInts); i++) {
    ...
    if (!sBlocked_.count(nId)) {
      ...
      if (pSubNtk) {
        Print(0, "got partitioning with node", nId, "(", i, "/",
              int_size(vInts), ")", "with size", pSubNtk->GetNumInts());
        return pSubNtk;
      }
    }
    vFailed_[nId] = true;
...
template <typename Ntk> void Partitioner<Ntk>::Insert(Ntk *pSubNtk) {
  PartitionIo &io = mSubNtk2Io_.at(pSubNtk);
  for (int nId : std::get<0>(io)) {
    sBlocked_.erase(nId);
  }
  std::pair<std::vector<int>, std::vector<bool>> vNewSignals =
      pNtk_->Insert(pSubNtk, std::get<1>(io), std::get<2>(io), std::get<3>(io));
...
template <typename Ntk> bool Partitioner<Ntk>::IsTooSmall(Ntk *pNtk) const {
...
template <typename Ntk>
void Partitioner<Ntk>::GetIo(const std::set<int> &sNodes,
                             std::set<int> &sInputs, std::set<int> &sOutputs) {
...
template <typename Ntk>
std::set<int> Partitioner<Ntk>::GetFanouts(const std::set<int> &sNodes,
                                           const std::set<int> &sOutputs) {
...
template <typename Ntk>
std::set<int> Partitioner<Ntk>::GetUnblockedNeighborsAndInners(int nId,
                                                               int nRadius) {
  ...
  while (!vInners.empty()) {
    ...
    Print(3, "inputs:", sInputs);
    Print(3, "outputs:", sOutputs);
    if (par_.nPartitionInputMax &&
        int_size(sInputs) > par_.nPartitionInputMax) {
      return std::set<int>();
    }
...
template <typename Ntk> Ntk *Partitioner<Ntk>::ExtractDisjoint(int nId) {
  ...
  mSubNtk2Io_.emplace(pSubNtkRaw,
                      std::make_tuple(std::move(sNodes), std::move(vInputs),
                                      std::vector<bool>(vInputs.size()),
                                      std::move(vOutputs)));
  return pSubNtkRaw;
}
