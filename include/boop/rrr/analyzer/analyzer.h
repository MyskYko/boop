...
template <typename Ntk, typename Sim, typename Sat> class Analyzer {
public:
  struct Parameter {
    int nVerbose = 0;
...
Analyzer<Ntk, Sim, Sat>::Analyzer(const Parameter &par)
    : pNtk_(nullptr), par_(par), sim_(par.parSim), sat_(par.parSat) {}
...
template <typename Ntk, typename Sim, typename Sat>
void Analyzer<Ntk, Sim, Sat>::SetPrintLine(
...
bool Analyzer<Ntk, Sim, Sat>::CheckRedundancy(int nId, int nIdx) {
  if (!sim_.CheckRedundancy(nId, nIdx)) {
    Print(1, "node", nId, ",", "fanin", pNtk_->GetCompl(nId, nIdx),
          pNtk_->GetFanin(nId, nIdx), ",", "index", nIdx, ":", "not redundant");
    return false;
  }
  Print(0, "node", nId, ",", "fanin", pNtk_->GetCompl(nId, nIdx),
        pNtk_->GetFanin(nId, nIdx), ",", "index", nIdx, ":",
        (sim_.IsExhaustive() ? "already redundant" : "seems redundant"));
  ...
  Print(0, "node", nId, ",", "fanin", pNtk_->GetCompl(nId, nIdx),
        pNtk_->GetFanin(nId, nIdx), ",", "index", nIdx, ":", "undetermined");
  return false;
}
...
bool Analyzer<Ntk, Sim, Sat>::CheckFeasibility(int nId, int nFi, bool fCompl) {
  ...
  Print(0, "node", nId, ",", "fanin", fCompl, nFi, ":",
        (sim_.IsExhaustive() ? "already feasible" : "seems feasible"));
  if (sim_.IsExhaustive()) {
    return true;
  }
