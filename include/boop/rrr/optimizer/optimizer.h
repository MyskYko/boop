...
template <typename Ntk, typename Ana> class Optimizer {
public:
  struct Parameter {
    int nVerbose = 0;
    int nSortType = -1;
...
private:
  ...
  // addition
  template <typename T> T SingleAdd(int nId, T begin, T end);
  ...
  void MultiTargetResubCleanup(const std::vector<int> &vTargets, int nSlot,
                               bool fTried, bool fAdded);
  ...
  void ApplyCombinationRandomly(
      int k, const std::function<bool(const std::vector<int> &)> &fn);
  void ApplyCombinationSampled(
      int k, const std::function<bool(const std::vector<int> &)> &fn);
  void
  ApplyMultisetSampled(int k,
  ...
  void RunHelper(
      ApplyFn &&fnApply,
      const std::vector<std::function<bool(int, const std::vector<int> &)>>
          &vResubs,
      const std::vector<std::string> &vNames, bool fStopAtChange,
      bool fRandomAddition);
};

// lifecycle

template <typename Ntk, typename Ana>
Optimizer<Ntk, Ana>::Optimizer(const Parameter &par,
                               std::function<Cost(Ntk *)> fnObjective)
    : pNtk_(nullptr), par_(par), nSortType_(par.nSortType), nTimeout_(0),
      fnObjective_(std::move(fnObjective)), nSamples_(-1), nTargets_(-1),
      nMax_(-1), ana_(par.parAna), nTarget_(-1) {}
...
void Optimizer<Ntk, Ana>::AssignNetwork(Ntk *pNtk, bool fReuse) {
  ...
  nMax_ = -1;
  nTarget_ = -1;
  pNtk_->AddCallback([this](const Action &action) { ActionCallback(action); });
  ana_.AssignNetwork(pNtk_, fReuse);
}

template <typename Ntk, typename Ana>
void Optimizer<Ntk, Ana>::SetPrintLine(
...
void Optimizer<Ntk, Ana>::Run(int nSeed, Seconds nTimeout) {
  ...
  case 0:
    RunHelper(
        [&](auto &&fn) {
          ApplyReverseTopologically(std::forward<decltype(fn)>(fn));
        },
        {[&](int nId, const std::vector<int> &vCands) {
          SingleResub(nId, vCands);
          return true;
        }},
        {"single"}, false, false);
    break;
  ...
  case 2: {
    ...
    while (true) {
      RunHelper(
          [&](auto &&fn) {
            ApplyReverseTopologically([&](int nId) { return fn(nId); });
          },
          {[&](int nId, const std::vector<int> &vCands) {
            SingleResub(nId, vCands);
            return true;
  ...
  case 3:
    RunHelper([&](auto &&fn) { ApplyRandomly(std::forward<decltype(fn)>(fn)); },
              {[&](int nId, const std::vector<int> &vCands) {
                 return SingleResubStop(nId, vCands);
               },
               [&](int nId, const std::vector<int> &vCands) {
                 return MultiResubStop(nId, vCands);
               }},
...
template <typename Ntk, typename Ana> void Optimizer<Ntk, Ana>::ResetSummary() {
  mStats_.clear();
  ana_.ResetSummary();
}
...
Summary<int> Optimizer<Ntk, Ana>::GetStatsSummary() const {
  ...
  for (const auto &entry : mStats_) {
    summary.emplace_back("opt " + entry.first + " tried node",
                         entry.second.nTried);
    summary.emplace_back("opt " + entry.first + " tried fanin",
                         entry.second.nTriedFis);
    summary.emplace_back("opt " + entry.first + " added node",
                         entry.second.nAdded);
    summary.emplace_back("opt " + entry.first + " added fanin",
                         entry.second.nAddedFis);
    summary.emplace_back("opt " + entry.first + " changed",
                         entry.second.nChanged);
...
Summary<Duration> Optimizer<Ntk, Ana>::GetTimesSummary() const {
  ...
  for (const auto &entry : mStats_) {
    summary.emplace_back("opt " + entry.first + " add",
                         entry.second.durationAdd);
    summary.emplace_back("opt " + entry.first + " reduce",
                         entry.second.durationReduce);
  }
...
template <typename Ntk, typename Ana> void Optimizer<Ntk, Ana>::SetRandCosts() {
  std::uniform_real_distribution<> dis(std::numeric_limits<double>::lowest(),
...
void Optimizer<Ntk, Ana>::SortFanins(int nId) {
  ...
  case 1: // prioritize internals
    pNtk_->SortFanins(
        nId, [&](int i, int j) { return !pNtk_->IsPi(i) && pNtk_->IsPi(j); });
    break;
  ...
  case 3: // prioritize internals with random PI order
    ...
    pNtk_->SortFanins(nId, [&](int i, int j) {
      if (pNtk_->IsPi(i) && pNtk_->IsPi(j)) {
        return vRandPiOrder_[pNtk_->GetPiIndex(i)] >
               vRandPiOrder_[pNtk_->GetPiIndex(j)];
      }
      return pNtk_->IsPi(j);
    });
    break;
  ...
  case 14: // random order
    SetRandCosts();
    pNtk_->SortFanins(
        nId, [&](int i, int j) { return vRandCosts_[i] > vRandCosts_[j]; });
    break;
  case 15: // random + PI
    SetRandCosts();
    pNtk_->SortFanins(nId, [&](int i, int j) {
...
template <typename Ntk, typename Ana> void Optimizer<Ntk, Ana>::SortFanins() {
  pNtk_->ForEachInt([&](int nId) { SortFanins(nId); });
}
...
bool Optimizer<Ntk, Ana>::RemoveRedundantFanins(int nId, bool fRemoveUnused) {
  ...
  for (int nIdx = 0; nIdx < pNtk_->GetNumFanins(nId); nIdx++) {
    ...
    if (ana_.CheckRedundancy(nId, nIdx)) {
      ...
      fReduced = true;
      nIdx--;
      if (fRemoveUnused && pNtk_->IsInt(nFi) &&
          pNtk_->GetNumFanouts(nFi) == 0) {
...
template <typename Ntk, typename Ana>
bool Optimizer<Ntk, Ana>::RemoveRedundantFaninsRandom(int nId,
                                                      bool fRemoveUnused) {
  assert(pNtk_->GetNumFanouts(nId) > 0);
  bool fReduced = false;
...
template <typename Ntk, typename Ana>
bool Optimizer<Ntk, Ana>::RemoveRedundancyOneTraversal(bool fRandom,
                                                       bool fSubRoutine) {
  TimePoint timeStart;
  if (!fSubRoutine) {
...
T Optimizer<Ntk, Ana>::SingleAdd(int nId, T begin, T end) {
  ...
  for (; it != end; ++it) {
    ...
    } else if (pNtk_->UseComplementedEdges() &&
               ana_.CheckFeasibility(nId, nCand, true)) {
...
template <typename Ntk, typename Ana>
void Optimizer<Ntk, Ana>::ResubCleanup(int nId, int nSlot, bool fTried,
                                       bool fAdded) {
...
void Optimizer<Ntk, Ana>::SingleResub(int nId, const std::vector<int> &vCands) {
  ...
  for (auto it = vCands.cbegin(); it != vCands.cend(); ++it) {
    ...
    fAdded = true;
    Print(2, "cand", *it, "(", int_distance(vCands.cbegin(), it) + 1, "/",
          int_size(vCands), ")", ":", "cost", "=", cost);
    if (Reduce()) {
      Cost costNew = fnObjective_(pNtk_);
...
template <typename Ntk, typename Ana>
bool Optimizer<Ntk, Ana>::SingleResubStop(int nId,
                                          const std::vector<int> &vCands) {
  assert(pNtk_->GetNumFanouts(nId) != 0);
  assert(pNtk_->GetNumFanins(nId) > 1);
  ...
  for (auto it = vCands.cbegin(); it != vCands.cend(); ++it) {
    ...
    fAdded = true;
    Print(3, "cand", *it, "(", int_distance(vCands.cbegin(), it) + 1, "/",
          int_size(vCands), ")", ":", "cost", "=", cost);
    if (Reduce()) {
      mNewFanins_.clear();
      Cost costNew = fnObjective_(pNtk_);
...
template <typename Ntk, typename Ana>
bool Optimizer<Ntk, Ana>::MultiResubStop(int nId,
                                         const std::vector<int> &vCands) {
  assert(pNtk_->GetNumFanouts(nId) != 0);
  assert(pNtk_->GetNumFanins(nId) > 1);
...
template <typename Ntk, typename Ana>
void Optimizer<Ntk, Ana>::MultiTargetTrivialCollapse(
...
template <typename Ntk, typename Ana>
bool Optimizer<Ntk, Ana>::MultiTargetAdd(const std::vector<int> &vTargets,
                                         bool fRandomAddition) {
  bool fAdded = false;
  for (int nId : vTargets) {
...
template <typename Ntk, typename Ana>
void Optimizer<Ntk, Ana>::MultiTargetResubCleanup(
    const std::vector<int> &vTargets, int nSlot, bool fTried, bool fAdded) {
...
template <typename Ntk, typename Ana>
void Optimizer<Ntk, Ana>::MultiTargetResub(std::vector<int> vTargets,
                                           bool fRandomAddition) {
  const int nSlot = pNtk_->Save();
  const Cost cost = fnObjective_(pNtk_);
...
template <typename Ntk, typename Ana>
bool Optimizer<Ntk, Ana>::MultiTargetResubStop(std::vector<int> vTargets,
                                               bool fRandomAddition) {
  const int nSlot = pNtk_->Save();
  const Cost cost = fnObjective_(pNtk_);
  MultiTargetTrivialCollapse(vTargets);
...
template <typename Ntk, typename Ana>
void Optimizer<Ntk, Ana>::ApplyReverseTopologically(
  ...
  for (auto it = vInts.crbegin(); it != vInts.crend(); ++it) {
    ...
    Print(1, "node", *it, "(", int_distance(vInts.crbegin(), it) + 1, "/",
          int_size(vInts), ")", ":", "cost", "=", fnObjective_(pNtk_));
    if (fn(*it)) {
      break;
    }
  }
...
void Optimizer<Ntk, Ana>::ApplyRandomly(const std::function<bool(int)> &fn) {
  ...
  for (auto it = vInts.cbegin(); it != vInts.cend(); ++it) {
    ...
    Print(1, "node", *it, "(", int_distance(vInts.cbegin(), it) + 1, "/",
          int_size(vInts), ")", ":", "cost", "=", fnObjective_(pNtk_));
    if (fn(*it)) {
      break;
    }
...
template <typename Ntk, typename Ana>
void Optimizer<Ntk, Ana>::ApplyCombinationRandomly(
    int k, const std::function<bool(const std::vector<int> &)> &fn) {
  std::vector<int> vInts = pNtk_->GetInts();
  std::shuffle(vInts.begin(), vInts.end(),
...
template <typename Ntk, typename Ana>
void Optimizer<Ntk, Ana>::ApplyCombinationSampled(
    int k, const std::function<bool(const std::vector<int> &)> &fn) {
...
template <typename Ntk, typename Ana>
void Optimizer<Ntk, Ana>::ApplyMultisetSampled(
    int k, const std::function<bool(const std::vector<int> &)> &fn) {
  const int nInts = pNtk_->GetNumInts();
  assert(nInts >= 1);
  std::vector<int> vInts = pNtk_->GetInts();
...
template <typename Ntk, typename Ana>
template <typename ApplyFn>
void Optimizer<Ntk, Ana>::RunHelper(
    ApplyFn &&fnApply,
    const std::vector<std::function<bool(int, const std::vector<int> &)>>
        &vResubs,
    const std::vector<std::string> &vNames, bool fStopAtChange,
