...
template <typename Ntk> class Simulator {
public:
  struct Parameter {
    int nVerbose = 0;
    int nWords = 16;
...
private:
  ...
  void PrintBits(int nVerboseLevel, int nWords,
...
template <typename Ntk>
void Simulator<Ntk>::SetPrintLine(
...
template <typename Ntk> bool Simulator<Ntk>::IsExhaustive() const {
  return fExhaustive_;
}
  ...
  case AND: {
    ...
    pNtk_->template ForEachFanin<true, true, false>(
        nId, [&](int nIdx2, int nFi, bool fCompl) {
          ...
          vec_ops::And(nWords_, vTmp_.begin(), it,
                       vValues_.begin() + nFi * nWords_, false, fCompl);
          it = vTmp_.begin();
        });
    int nFi = pNtk_->GetFanin(nId, nIdx);
    bool fCompl = pNtk_->GetCompl(nId, nIdx);
    vec_ops::And(nWords_, vTmp_.begin(), it, vValues_.begin() + nFi * nWords_,
                 false, !fCompl);
...
bool Simulator<Ntk>::CheckFeasibility(int nId, int nFi, bool fCompl) {
  ...
  case AND: {
    ...
    pNtk_->ForEachFanin(nId, [&](int nFi2, bool fCompl2) {
      vec_ops::And(nWords_, vTmp_.begin(), it,
                   vValues_.begin() + nFi2 * nWords_, false, fCompl2);
      it = vTmp_.begin();
    });
    vec_ops::And(nWords_, vTmp_.begin(), it, vValues_.begin() + nFi * nWords_,
                 false, !fCompl);
            ...
            pNtk_->ForEachFanin(nId, [&](int nFi, bool fCompl) {
              if (itX == vValuesInv_.end()) {
                ...
                if (vTrav_[nFi] != nTrav_) {
                  ...
                  vec_ops::And(1, itY, itX,
                               vValuesInv_.begin() + nFi * nWords_ + nWord,
                               fComplX, fCompl);
                }
                itX = itY;
                fComplX = false;
...
template <typename Ntk> void Simulator<Ntk>::ResetSummary() {
  nCex_ = 0;
  nDiscarded_ = 0;
  nPackedCountOld_ = 0;
...
template <typename Ntk> Summary<int> Simulator<Ntk>::GetStatsSummary() const {
  Summary<int> summary;
...
template <typename Ntk>
void Simulator<Ntk>::PrintBits(int nVerboseLevel, int nWords,
                               std::vector<Word>::const_iterator it) {
  ...
  case AND:
    pNtk_->ForEachFanin(nId, [&](int nFi, bool fCompl) {
      if (itX == v.end()) {
        ...
        vec_ops::And(nWords_, itY, itX, v.begin() + nFi * nWords_, fComplX,
                     fCompl);
        itX = itY;
        fComplX = false;
      }
  ...
  case AND:
    pNtk_->ForEachFanin(nId, [&](int nFi, bool fCompl) {
      if (itX == v.end()) {
        ...
        vec_ops::And(nWords_, vTmp_.begin(), itX, v.begin() + nFi * nWords_,
                     fComplX, fCompl);
        itX = vTmp_.begin();
        fComplX = false;
      }
...
template <typename Ntk>
void Simulator<Ntk>::SimulateOneWordNode(std::vector<Word> &v, int nId,
                                         int nOffset) {
  auto itX = v.end();
  ...
  case AND:
    pNtk_->ForEachFanin(nId, [&](int nFi, bool fCompl) {
      if (itX == v.end()) {
        ...
        vec_ops::And(1, itY, itX, v.begin() + nFi * nWords_ + nOffset, fComplX,
                     fCompl);
        itX = itY;
        fComplX = false;
      }
...
template <typename Ntk> void Simulator<Ntk>::Resimulate() {
  ...
  { SimulateNode(vValues_, nId); Print(1, "resimulating", "node", nId);
    PrintBits(2, nWords_, vValues_.begin() + nId * nWords_);
    }
  });
  */
...
template <typename Ntk> void Simulator<Ntk>::GenerateRandomStimuli() {
...
template <typename Ntk> void Simulator<Ntk>::GenerateExhaustiveStimuli() {
...
template <typename Ntk> void Simulator<Ntk>::ComputeCare(int nId) {
      ...
      pNtk_->ForEachFanin(nId, [&](int nFi, bool fCompl) {
        if (itX == vValuesInv_.end()) {
          ...
          if (vTrav_[nFi] != nTrav_) {
            ...
            vec_ops::And(nWords_, itY, itX, vValuesInv_.begin() + nFi * nWords_,
                         fComplX, fCompl);
          }
          itX = itY;
          fComplX = false;
...
template <typename Ntk> void Simulator<Ntk>::Initialize() {
  if (!fGenerated_) {
    nWords_ = par_.nWords;
    if (nWords_ == 0 || pNtk_->GetNumPis() > 36 ||
...
template <typename Ntk> void Simulator<Ntk>::Save(int nSlot) {
  assert(nSlot >= 0);
  assert(!check_int_max(
      nSlot)); // TODO: revisit this after network save behavior is decided
...
template <typename Ntk> void Simulator<Ntk>::Load(int nSlot) {
  assert(nSlot >= 0);
  ...
  for (int i = 0; i < nWords_; i++) {
    ...
    pNtk_->ForEachPi([&](int nId) { // TODO: break maybe
      if (vBackups_[nSlot].vValues_[nId * vBackups_[nSlot].nWords_ + i] !=
          vValues_[nId * nWords_ + i]) {
        fDifferent = true;
      }
    });
    if (fDifferent) {
  ...
  if (vOffsets.empty()) {
    ...
    pNtk_->ForEachPi([&](int nIdx, int nId) {
      ...
      vec_ops::Copy(nWords_, vInputStimuli[nIdx].begin(),
                    vValues_.begin() + nId * nWords_, false);
    });
    vValues_ = vBackups_[nSlot].vValues_;
    pNtk_->ForEachPi([&](int nIdx, int nId) {
      vec_ops::Copy(nWords_, vValues_.begin() + nId * nWords_,
                    vInputStimuli[nIdx].begin(), false);
    });
    for (int nOffset : vOffsets) {
      SimulateOneWord(nOffset);
    }
  }
  // TODO: better to make network class clear out all backups on read unless
  // fReuse (also on assign network?)
}

template <typename Ntk> void Simulator<Ntk>::PopBack() { vBackups_.pop_back(); }
