...
namespace boop::vec_ops {
  ...
  inline void Clear(int n, It it) {
    using T = typename std::iterator_traits<It>::value_type;
    std::fill(it, it + n, T{0});
  }

  template <typename It>
  inline void Fill(int n, It it) {
    using T = typename std::iterator_traits<It>::value_type;
    std::fill(it, it + n, ~T{0});
  }
  ...
  inline void And(int n, ItDst itDst, ItSrc0 itSrc0, ItSrc1 itSrc1, bool fCompl0, bool fCompl1) {
    if(!fCompl0) {
      if(!fCompl1) {
  ...
  template <typename ItDst, typename ItSrc0, typename ItSrc1>
  inline void Xor(int n, ItDst itDst, ItSrc0 itSrc0, ItSrc1 itSrc1, bool fCompl1) {
    if(!fCompl1) {
      for(int i = 0; i < n; i++, ++itDst, ++itSrc0, ++itSrc1) {
        *itDst = (*itSrc0) ^ (*itSrc1);
      }
  ...
  template <typename It>
  inline bool IsZero(int n, It it, bool fCompl) {
    if(!fCompl) {
  ...
  template <typename It>
  inline std::stringstream GetStringStream(int n, It it) {
