...
namespace boop::rrr {
  ...
  void SatBasedSolver<Ntk, Sol>::AssignNetwork(Ntk *pNtk, bool fReuse) {
    ...
    fUpdate_ = false;
    pNtk_ = pNtk;
    pNtk_->AddCallback(std::bind(&SatBasedSolver<Ntk, Sol>::ActionCallback, this, std::placeholders::_1));
  }

  template <typename Ntk, typename Sol>
  void SatBasedSolver<Ntk, Sol>::SetPrintLine(std::function<void(const std::string &)> fnPrintLine) {
  ...
  template <typename Ntk, typename Sol>
  void SatBasedSolver<Ntk, Sol>::Justify(std::vector<VarValue> &vValues, int nId) {
  ...
  std::vector<VarValue> SatBasedSolver<Ntk, Sol>::GetCex() {
    if(par_.nVerbose) {
      std::stringstream ss;
      pNtk_->ForEachPi([&](int nId) {
        ss << GetVarValueChar(vValues_[nId]);
      });
      Print(0, "cex:", ss.str());
    }
    // reverse simulation
    if(fNoDontCare_) {
      ...
      pNtk_->template ForEachInt<true>([&](int nId) {
        ...
        if(vValuesInv_[nId] == rrrTRUE || vValuesInv_[nId] == rrrFALSE) {
          assert(nId == nTarget_ || vValuesInv_[nId] == DecideVarValue(vValues_[nId]));
          vValues_[nId] = DecideVarValue(vValues_[nId]);
        }
        Justify(vValues_, nId);
      });
  ...
  void SatBasedSolver<Ntk, Sol>::ActionCallback(const Action &action) {
    ...
    case TRIVIAL_DECOMPOSE:
      fUpdate_ = true; // necessary if function or don't care of new edge needs to be considered
      break;
    case SORT_FANINS:
      break;
    case READ:
  ...
  void SatBasedSolver<Ntk, Sol>::EncodeMiter() {
    ...
    pNtk_->ForEachPi([&](int nId) {
      vVarLits_[nId] = sol_.NewVar();
    });
    Print(0, "encoding network");
    pNtk_->ForEachInt([&](int nId) {
      EncodeNode(vVarLits_, nId);
    });
