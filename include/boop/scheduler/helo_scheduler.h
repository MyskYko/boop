...
template <typename Ntk, typename Opt, typename Prt> class HeloScheduler {
public:
  struct Parameter {
...
private:
  static constexpr const char *pCompress2rs =
      "balance -l; resub -K 6 -l; rewrite -l; resub -K 6 -N 2 -l; refactor -l; "
      "resub -K 8 -l; balance -l; resub -K 8 -N 2 -l; rewrite -l; resub -K 10 "
      "-l; rewrite -z -l; resub -K 10 -N 2 -l; balance -l; resub -K 12 -l; "
  ...
  std::priority_queue<Job *, std::vector<Job *>, CompareJobPointers>
      qFinishedJobs_;
  std::mutex mutexAbc_;
  ...
  std::string MakeStepInfoString(const Ntk *pNtk, Cost cost, Cost costInitial,
  ...
  // summary
  template <typename T>
  void AddToSummary(std::vector<std::string> &vKeys,
                    std::map<std::string, T> &mSummary,
                    const Summary<T> &summary) const;
...
HeloScheduler<Ntk, Opt, Prt>::HeloScheduler(Ntk *pNtk, Parameter const &par)
    : pNtk_(pNtk), par_(par), fMultithreaded_(par_.nThreads > 1),
      fDeterministic_(par_.fDeterministic), prt_(par_.parPrt), pOpt_(nullptr),
      nCreatedJobs_(0), nFinishedJobs_(0) {
#ifdef BOOP_USE_THREADS
  fTerminate_ = false;
  if (fMultithreaded_) {
    vThreads_.reserve(par_.nThreads);
    for (int i = 0; i < par_.nThreads; i++) {
      vThreads_.emplace_back(
          [this, parOpt = par_.parOpt]() { Thread(parOpt); });
    }
    return;
  }
#endif
...
template <typename Ntk, typename Opt, typename Prt>
template <typename... Args>
void HeloScheduler<Ntk, Opt, Prt>::Print(int nVerboseLevel,
...
template <typename Ntk, typename Opt, typename Prt>
void HeloScheduler<Ntk, Opt, Prt>::ExecuteTranstochFlow(Opt &opt, Job *pJob,
...
template <typename Ntk, typename Opt, typename Prt>
void HeloScheduler<Ntk, Opt, Prt>::ExecuteDeepFlow(Opt &opt, Job *pJob,
                                                   Duration &durationAbc) {
  SimpleRNG rng;
  for (int i = 0; i < 11; i++) {
  ...
  for (int i = 0; i < 1000000; i++) {
    ...
    std::string strComp;
    if (nComp == 3) {
      strComp = std::string("; &put; ") + pCompress2rs + "; " + pCompress2rs +
                "; " + pCompress2rs + "; &get";
    } else if (nComp == 2) {
      strComp = std::string("; &put; ") + pCompress2rs + "; " + pCompress2rs +
                "; &get";
...
template <typename Ntk, typename Opt, typename Prt>
void HeloScheduler<Ntk, Opt, Prt>::OnJobEnd(
    const std::function<void(Job *pJob)> &fn) {
#ifdef BOOP_USE_THREADS
  if (fMultithreaded_) {
    Job *pJob = nullptr;
    {
      std::unique_lock<std::mutex> l(mutexFinishedJobs_);
      while (qFinishedJobs_.empty() ||
             (fDeterministic_ && qFinishedJobs_.top()->nId != nFinishedJobs_)) {
...
#ifdef BOOP_USE_THREADS
template <typename Ntk, typename Opt, typename Prt>
void HeloScheduler<Ntk, Opt, Prt>::Thread(
    const typename Opt::Parameter &parOpt) {
  StartAbc();
...
template <typename Ntk, typename Opt, typename Prt>
template <typename T>
void HeloScheduler<Ntk, Opt, Prt>::AddToSummary(
    std::vector<std::string> &vKeys, std::map<std::string, T> &mSummary,
    const Summary<T> &summary) const {
...
void HeloScheduler<Ntk, Opt, Prt>::RunWithPartitioning() {
  ...
  fDeterministic_ =
      false; // deterministic anyway (wait for all jobs each round)
  pNtk_->Sweep();
  prt_.AssignNetwork(pNtk_);
  prt_.SetPrintLine([&](const std::string &str) { Print(-1, "", str); });
  while (nCreatedJobs_ < par_.nJobs) {
    ...
    if (nCreatedJobs_ < nFinishedJobs_ + par_.nParallelPartitions) {
      ...
      if (pSubNtk) {
        ...
        Print(1, pJob->strPrefix, "created", ":",
              MakeNtkInfoString(pJob->pNtk, pJob->costInitial));
        continue;
      }
    }
    ...
    while (nFinishedJobs_ < nCreatedJobs_) {
      OnJobEnd([&](Job *pJob) {
        ...
        Print(1, pJob->strPrefix, "finished", ":",
              MakeNtkInfoString(pJob->pNtk, cost));
        Print(0, "", "job", pJob->nId, "(", nFinishedJobs_ + 1, "/", par_.nJobs,
              ")", ":",
              MakeStepInfoString(pJob->pNtk, cost, pJob->costInitial,
                                 pJob->duration));
    ...
    if (par_.fOptOnInsert) {
      ...
      Duration duration = 0;
      CallAbc(pNtk_, std::string("&put; ") + pCompress2rs + "; dc2; &get",
              duration);
      prt_.AssignNetwork(pNtk_);
      Cost cost = par_.fnObjective(pNtk_);
      Print(0, "", "c2rs; dc2", ":",
