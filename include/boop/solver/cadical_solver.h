...
public:
  ...
  template <class It> void AddClause(It it, It itEnd);
  bool IsInconsistent();
...
CadicalSolver::CadicalSolver()
    : nVars_(0), nClauses_(0), pSolver_(std::make_unique<Solver>()) {}
...
template <class It> void CadicalSolver::AddClause(It it, It itEnd) {
  for (; it != itEnd; ++it) {
    pSolver_->add(*it);
  }
  pSolver_->add(0);
...
bool CadicalSolver::IsInconsistent() { return pSolver_->inconsistent(); }
...
Status CadicalSolver::Solve(const std::vector<int> *vAssumptions,
                            std::set<int> *sCore) {
