...
#else
template <typename Fn, typename... Args> struct invoke_return {
private:
  template <typename F, typename... A>
  static auto test(int) -> decltype(std::declval<F>()(std::declval<A>()...));
  template <typename, typename...> static void test(...);
...
#if defined(__cpp_lib_is_invocable)
template <typename Fn, typename... Args>
constexpr bool returns_int_v =
    std::is_same_v<invoke_return_t<Fn, Args...>, int>;
#else
template <typename Fn, typename... Args>
...
constexpr bool returns_bool_v =
    std::is_same_v<invoke_return_t<Fn, Args...>, bool>;
#else
template <typename Fn, typename... Args>
